...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry-$TARGETARCH \
    --mount=type=cache,target=/src/target,id=cargo-target-search-proxy-$TARGETARCH \
    cargo build --release --locked -p search-proxy \
    && cp target/release/search-proxy /search-proxy

FROM scratch
# The CA bundle, because the proxy speaks TLS to a managed backend. `scratch` has no certificates,
# and the failure without this is a handshake error that reads like a network problem.
COPY --from=build /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
COPY --from=build /search-proxy /search-proxy
