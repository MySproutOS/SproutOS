...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry-$TARGETARCH \
    --mount=type=cache,target=/src/target,id=cargo-target-valkey-proxy-$TARGETARCH \
    cargo build --release --locked -p valkey-proxy \
    && cp target/release/valkey-proxy /valkey-proxy
...
COPY --from=build /valkey-proxy /valkey-proxy
