...
the launcher's `--share-net` choice. A launch-template change triggers a rolling ASG refresh, so a
host-policy repair reaches running instances. Empty exit-1 responses with stderr are surfaced as
launcher failures. Signed template installs pass their exact manifest commit to snapshot-copy, and
a regression advances the upstream branch before asserting that the target still contains the
pinned tree.
