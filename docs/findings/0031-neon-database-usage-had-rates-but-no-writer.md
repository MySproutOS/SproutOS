# 0031 — Neon database usage had rates but no writer
...
path, while official Neon documentation proves the API contract. It is not proof that a production
paid-plan credential has returned consumption or that the job has run after deployment.
