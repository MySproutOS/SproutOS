...
case "$2" in
  ...
  describe-auto-scaling-groups) echo "${STUB_ATTACHED:-True}" ;;
...
run() {
  ...
  export STUB_LIVE STUB_ATTACHED STUB_HEALTHY
  bash "$HERE/enable-storage-proxy.sh" 2>&1
}

echo "enable-storage-proxy.sh"
STUB_LIVE=arn:router-blue STUB_ATTACHED=True STUB_HEALTHY=1 out=$(run); status=$?
...
STUB_LIVE=arn:router-blue STUB_ATTACHED=False STUB_HEALTHY=1 out=$(run); status=$?
check "refuses before OpenTofu attaches the target group" 1 "$status"
check "does not move an unattached target" 0 "$(wc -l < "$STUB_CALLS" | tr -d ' ')"

STUB_LIVE=arn:router-green STUB_ATTACHED=True STUB_HEALTHY=0 out=$(run); status=$?
check "refuses an attached but unhealthy target" 1 "$status"
check "does not move an unhealthy target" 0 "$(wc -l < "$STUB_CALLS" | tr -d ' ')"

STUB_LIVE=arn:unknown STUB_ATTACHED=True STUB_HEALTHY=1 out=$(run); status=$?
