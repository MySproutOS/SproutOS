 ...
 * request/byte metrics are therefore compared with deduplicated ClickHouse events for each closed
 * UTC day. A positive residual is an operational/platform-overhead fact, never a usage event.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema
    ...
    .addColumn("provider_egress_bytes", "numeric(38, 9)", (col) => col.notNull())
    .addColumn("imported_requests", "numeric(38, 9)", (col) => col.notNull())
    .addColumn("imported_egress_bytes", "numeric(38, 9)", (col) => col.notNull())
    .addColumn("residual_requests", "numeric(38, 9)", (col) => col.notNull())
    .addColumn("residual_egress_bytes", "numeric(38, 9)", (col) => col.notNull())
    ...
    .addCheckConstraint(
      ...
      sql`
        provider_requests >= 0 and provider_egress_bytes >= 0 and
        imported_requests >= 0 and imported_egress_bytes >= 0 and
        residual_requests >= 0 and residual_egress_bytes >= 0
      `,
    )
    .execute()
}
