...
export default defineConfig(({ mode }) => {
  /*
    Read once, up here, because two separate things need it and `loadEnv` re-reads the file each
    call. `""` as the prefix is what makes it return non-`VITE_` names — the default prefix would
    hide `NEXT_PUBLIC_API_URL` entirely and this would look like a variable nobody set.
  */
  ...
  return {
    ...
    */
    ...
    /*
      ...
      both work.
    */
    base: env.VITE_ASSET_BASE ?? "/admin/",
    /*
      Two compile-time constants Vite does not supply on its own: the API host, and — measured, not
      assumed — `NODE_ENV`, without which `vite build` ships React's development build. See the
      helper; both were found by reading a bundle, and both were wrong while the app still worked.
    */
    ...
    resolve: {
      alias: {
        ...
        "@ui/seo-shared/_internal/seo-link": path.resolve(
          __dirname,
          "./src/components/seo-link.tsx",
        ),
