...
export const postV1OrgsByOrgSlugProjectsByProjectIdJobsByJobIdCancelResponseTransformer = async (
  data: any,
): Promise<PostV1OrgsByOrgSlugProjectsByProjectIdJobsByJobIdCancelResponse> => {
  if (data.startedAt) {
    data.startedAt = new Date(data.startedAt)
  }
  if (data.finishedAt) {
...
export const getV1UserSignInMethodsResponseTransformer = async (
  data: any,
): Promise<GetV1UserSignInMethodsResponse> => {
  data.data = data.data.map((item: any) => {
    item.connectedAt = new Date(item.connectedAt)
    return item
  })
  return data
}
