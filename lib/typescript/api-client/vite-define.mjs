  ...
  */
  ...
  return {
    ...
    /*
      ...
      It must also be in `passThroughEnv` for the `build` task in `turbo.json`, and that is not
      optional. Turbo runs tasks in a filtered environment and infers per-framework prefixes —
      `NEXT_PUBLIC_*` for Next.js and `VITE_*` for Vite pass automatically, and a name matching
      neither is dropped before Vite ever sees it. The first deployment of this proved it: the
      workflow set `STRIPE_PUBLIC_KEY`, the build succeeded, and the published bundle contained
      `api.sproutos.me` and no key at all — so the button shipped permanently disabled with no
      error anywhere. Reading the bundle is what found it; the exit code was zero throughout.
