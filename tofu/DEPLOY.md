...
The GitHub `cli-release-production` environment must exist with required reviewers before the
promotion workflow is used. Do not add that reviewer rule to the shared `production` environment:
automatic main deployments use it and must not acquire a manual gate. Promotion assumes both its
narrow AWS role and the existing deploy role only through the dedicated environment; the promotion
role does not trust the shared environment or an unprotected `cli-v*` ref directly, and the tag
publication workflow has no AWS authority. After this trust change is applied, inspect both roles'
subjects and verify that the repository-name and exact repository-ID forms name
